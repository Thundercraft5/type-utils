...
type NormalizeComparison<T, B> = {
	[K in keyof B]: Literal<B[K]> extends B[K]
		? B[K]
		: B[K] extends object
			? NormalizeComparison<T[K & keyof T], B[K]>
			: Widen<T[K & keyof T]>
};
export type Restrict<T extends any, Bounds extends unknown> =
	T extends Bounds
		? T extends Function | Narrowable
			? T
			: T extends object
				? Bounds extends NormalizeComparison<T, Bounds>
					? T
					: Bounds
				: T
		: Bounds;

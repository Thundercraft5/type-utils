...
> = {
	[K in keyof O]: O[K] extends GenericObject
		? NestedObjectPaths<O[K], Union<Prev, Path>, Join<Path, K>>
 ...
 * // { c: string }
 */
export type ValueFromNestedPath<
	O extends GenericObject,
	Path extends string, // Or, if you prefer, NestedPaths<T>
> = {
	[K in Path]: K extends keyof O
		? O[K]
		: K extends `${ infer P }.${ infer S }`
			? O[P] extends GenericObject
				? ValueFromNestedPath<O[P], S>
				: never
			: never;
}[Path];
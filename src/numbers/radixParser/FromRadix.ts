...
type FromRadix$1<E extends [number, number][], R extends keyof Numbers = 10> =
	E extends [
		[infer E extends number, infer N extends number],
		...infer T extends [number, number][],
	]
		? [Multiply<N, Exponent<R, E>>, ...FromRadix$1<T, R>]
		: [];
type FromRadix$2<S extends string, R extends keyof Numbers> = S extends `${
		infer N extends Numbers[Exclude<keyof Numbers, Range<R, 36>>]
	}${ infer T }`
	? [LettersMap[N & keyof LettersMap], ...FromRadix$2<T, R>]
	: [];
 ...
 */	
export type FromRadix<S extends string, R extends keyof Numbers> = AddAll<
		FromRadix$1<Extract<
			ReverseTuple<Extract<
				Entries<ReverseTuple<FromRadix$2<S, R>>>,
			any[]>>,
			[number, number][]>,
		R>
	>;

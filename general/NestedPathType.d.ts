import { ExactKeyof } from "../objects";
 ...
 */
export type NestedPathType<O, Path extends NestedObjectPaths<O>> = {
    [K in Path]: K extends keyof OnlyObject<O> ? OnlyObject<O>[K] | NullableIfNotObject<O> : K extends `[]${infer S}` ? O extends any[] ? S extends `${OptionalSep}${infer R extends NestedObjectPaths<OnlyObject<O[number]>>}` ? NestedPathType<OnlyObject<O[number]>, TrimSep<S> & R> | PreserveNullIndex<O, number> : O[number] | PreserveNullIndex<O, number> : S extends `${OptionalSep}${infer R extends Exclude<NestedObjectPaths<OnlyObject<O>>, `${bigint}`>}` ? NestedPathType<OnlyObject<O>, TrimSep<S> & R> | PreserveNull<O> : never : K extends `[${infer I extends Extract<keyof OnlyObject<O>, `${bigint}`>}]${infer S}` ? S extends `${OptionalSep}${infer R extends NestedObjectPaths<OnlyObject<O>[I]>}` ? NestedPathType<OnlyObject<O>[I], TrimSep<S> & R> | PreserveNullIndex<O, I & ExactKeyof<O>> : OnlyObject<O>[I] | PreserveNullIndex<O, I & ExactKeyof<O>> : K extends `${infer P extends Exclude<keyof O, symbol>}${infer S}` ? S extends `${OptionalSep}${infer R extends NestedObjectPaths<OnlyObject<O>[P]>}` ? NestedPathType<OnlyObject<O>[P], TrimSep<S> & R> | PreserveNullIndex<O, P & ExactKeyof<P> & keyof O> : never : K;
}[Path];
export {};
//# sourceMappingURL=NestedPathType.d.ts.map